...
gcloud run jobs $ACTION "${JOB}" \
  ...
  --args="^|^${JOINED}" \
  --service-account="${SA}" \
  --set-env-vars="CASEOPS_ENV=cloud,CASEOPS_AUTO_MIGRATE=false,CASEOPS_LLM_PROVIDER=openai,CASEOPS_LLM_MODEL=gpt-5.1,CASEOPS_LAYER2_DAILY_USD_CAP=20,CASEOPS_LAYER2_DAILY_CAP_USD=20,CASEOPS_EMBEDDING_PROVIDER=voyage,CASEOPS_EMBEDDING_MODEL=voyage-4-large,CASEOPS_EMBEDDING_DIMENSIONS=1024" \
