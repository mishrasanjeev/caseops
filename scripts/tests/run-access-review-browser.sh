#!/usr/bin/env bash
set -euo pipefail
: "${RUN_LABEL:?Use a fresh retained evidence label}"
test ! -e "/output/${RUN_LABEL}.json"
test ! -e "/output/${RUN_LABEL}-build.txt"
test "$(df -Pk /scratch | awk 'NR == 2 {print $4}')" -ge 2097152
cp /bin/true /scratch/exec-probe
chmod 700 /scratch/exec-probe
/scratch/exec-probe
...
cd ../..
node node_modules/@playwright/test/cli.js test --config playwright.access-reviews.config.ts
