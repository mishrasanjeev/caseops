...
def build_summary_sheet(wb: Workbook) -> None:
    ...
    rows = [
        ...
        ["Total PRs opened", "5 fix PRs (#20, #21, #22, #23, #24) + closeout (#25) + dependabot pre-cursor (#18)"],
        ["Verdict status (per bug-fixing skill)", "All five MERGED to main on 2026-05-10. Verdicts remain Partially fixed pending production deploy + prod-Playwright on the deployed SHA."],
        ["Final main SHA after Hari batch", "ed01fdd (after #24 squash-merged 2026-05-10)"],
        ["Merge SHAs", "#18→b827efb · #20→78108e4 · #21→877c615 · #22→2b571cc · #23→7761a1b · #24→ed01fdd"],
        ...
        ["Closeout PR", "#25 — this workbook + docs updates"],
        [],
...
def build_bug_details_sheet(wb: Workbook) -> None:
    ...
    rows = [
        [
            ...
            "Partially fixed pending deploy + prod-Playwright run on deployed SHA (PR merged)",
...
def build_pending_prod_proof_sheet(wb: Workbook) -> None:
    ...
    rows = [
        ...
        ["BUG-032", "#24", "Deploy via scripts/deploy-prod.sh", "Operator", "Awaiting deploy approval", "Deploy script exit 0; Cloud Run revision tag matches deployed main SHA."],
