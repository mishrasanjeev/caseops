...
def build_summary_sheet(wb: Workbook) -> None:
    ...
    rows = [
        ...
        ["Total PRs opened", "5 fix PRs (#20, #21, #22, #23, #24) + closeout #25 + dependabot pre-cursor #18 + URL-fix follow-up #26 + final-verdicts (this PR)"],
        ["Verdict status", "BUG-032/033/034/039 → Properly fixed (prod-Playwright passed on deployed SHA). BUG-038 → Partially fixed: provider/runtime/live-ingestion all verified (dashboard enabled, public key present, SendGrid test API returned 204, real Event API POSTs returning 200 with active sig verification); sole remaining caveat is DB-side proof of a tenant-scoped EmailSuppression row from a real production bounce/unsubscribe event."],
        ["Final main SHA deployed", "ec55dc2 (after #26 merged 2026-05-10)"],
        ["Merge SHAs", "#18→b827efb · #20→78108e4 · #21→877c615 · #22→2b571cc · #23→7761a1b · #24→ed01fdd · #25→7c1934c · #26→ec55dc2"],
        ["Pre-cursor merge", "#18 (dependabot langchain-core 1.3.3) merged first to clear CVE-2026-44843; without it, every Hari PR's pip-audit stayed red."],
        ["URL-fix follow-up", "#26 corrected stale /api/sendgrid/events references in workbook + audit docs → canonical /api/webhooks/sendgrid/events. Runbook was already correct; app code unchanged."],
        ["Production deploy", "scripts/deploy-prod.sh ec55dc2 — exit 0. caseops-api revision caseops-api-00132-c6n (image :ec55dc2). caseops-web revision caseops-web-00121-278 (image :ec55dc2). Health {\"status\":\"ok\"}. EG-003 ClamAV sidecar regression guard passed."],
        ["Prod-Playwright run", "GH Actions workflow run 25628453223 (Prod verification (Playwright)) — 38 passed (13.4m) against deployed ec55dc2. All 4 Hari prod specs green: 2/2 BUG-032 + 3/3 BUG-033 + 2/2 BUG-034 + 2/2 BUG-039."],
        ["SendGrid runtime readiness", "CASEOPS_SENDGRID_WEBHOOK_PUBLIC_KEY env wired via secretKeyRef name=caseops-sendgrid-webhook-public-key key=latest on caseops-api. Fail-closed verified: invalid + missing signatures both → HTTP 401 at canonical URL https://api.caseops.ai/api/webhooks/sendgrid/events. Secret value never read or printed."],
        ["SendGrid provider verification (2026-05-10)", "Dashboard ON; URL exact match https://api.caseops.ai/api/webhooks/sendgrid/events; Signed Event Webhook ON; events enabled = delivered/bounce/dropped/spam_report/unsubscribe/group_unsubscribe; public_key_present=true. SendGrid official webhook test API returned HTTP 204 (settings id c3d614d6-40fa-4db7-b295-c3e6cf44cd92)."],
        ["SendGrid live ingestion proof (2026-05-10)", "Cloud Run logs show real User-Agent='SendGrid Event API' POSTs to /api/webhooks/sendgrid/events returning HTTP 200. Samples: 15:42:14Z 0.162s, 15:40:03Z 0.094s, 15:35:17Z 1.738s."],
        ["SendGrid remaining caveat", "DB-side proof of a tenant-scoped EmailSuppression row written from a real production bounce/unsubscribe event. The 200 status is strong indirect evidence; the strict bug-fixing skill's verdict-of-record holds Partially fixed until the row is directly observed."],
        [],
        ["Key cross-PR risk RESOLVED", "playwright.app.config.ts and playwright.prod-ram.config.ts both gained testMatch entries from PRs #20, #21, #23, #24. All 4 hari-2026-05-09-*.spec.ts entries (and 4 prod variants) now present in main as of ec55dc2 — verified via the prod-Playwright workflow run that exercised all 4 specs."],
...
def build_bug_details_sheet(wb: Workbook) -> None:
    ...
    rows = [
        [
            ...
            "Merge SHA ed01fdd (in main as of 2026-05-10).",
            "https://github.com/mishrasanjeev/caseops/pull/24",
            "Properly fixed — prod-Playwright passed on deployed SHA ec55dc2 via workflow run 25628453223 on 2026-05-10.",
        ...
        [
            ...
            "Local + prod-Playwright (prod run pending deploy of merged SHA).",
            "Merge SHA 877c615 (in main as of 2026-05-10).",
        ...
        [
            ...
            "Local + prod-Playwright (prod run pending deploy of merged SHA).",
            "Merge SHA 78108e4 (in main as of 2026-05-10).",
        ...
        [
            ...
            "Partially fixed — runtime + provider + live ingestion all verified on deployed SHA ec55dc2; only DB-side suppression-row proof from a real production event remains. Runtime: CASEOPS_SENDGRID_WEBHOOK_PUBLIC_KEY env wired (secretKeyRef name=caseops-sendgrid-webhook-public-key key=latest); fail-closed verified (invalid + missing sigs → 401 at canonical URL https://api.caseops.ai/api/webhooks/sendgrid/events). Provider: SendGrid dashboard ON with all 6 events enabled (delivered/bounce/dropped/spam_report/unsubscribe/group_unsubscribe), public_key_present=true, SendGrid test API returned HTTP 204 (settings id c3d614d6-40fa-4db7-b295-c3e6cf44cd92). Live: Cloud Run logs show real User-Agent='SendGrid Event API' POSTs returning HTTP 200 (samples 2026-05-10T15:42:14Z 0.162s, 15:40:03Z 0.094s, 15:35:17Z 1.738s). Remaining caveat (sole reason verdict not graduated): a database-side query showing an EmailSuppression row written from a real bounce/unsubscribe event. Strict bug-fixing skill holds Partially fixed until the row is directly observed.",
        ...
        [
            ...
            "Local + prod-Playwright (prod run pending deploy of merged SHA).",
            "Merge SHA 7761a1b (in main as of 2026-05-10).",
...
def build_verification_matrix_sheet(wb: Workbook) -> None:
    ...
    rows = [
        [
            ...
            "Backend 7/7 green; vitest page.test 5/5 green; local Playwright suite green on rebased HEAD; CI all-green and squash-merged.",
            "Pending — runs after deploy via Prod verification (Playwright) workflow on deployed ed01fdd-or-later SHA.",
        ],
        [
            "BUG-033",
            "#21",
            ...
            "Backend green (auth + mailer); vitest green; local Playwright route-renders green; CI all-green and squash-merged.",
            "Pending — runs after deploy.",
        ],
        [
            "BUG-034",
            "#20",
            ...
            "Backend green; vitest green; local Playwright catalog-shape + UI-disabled green; CI all-green and squash-merged (one CI variance recovered via single approved rerun).",
            "Pending — runs after deploy.",
        ],
        [
            "BUG-038 (SendGrid)",
            ...
            "12/12 backend tests green; Alembic upgrade + downgrade dry-run both clean against SQLite; CI all-green and squash-merged.",
        ...
        [
            ...
            "Backend green; vitest green; local Playwright endpoint-shape green; CI all-green and squash-merged.",
            "Pending — runs after deploy.",
        ],
        [
            "Pre-cursor: langchain-core CVE bump",
            "#18",
            "b827efb",
            "dependabot/uv/apps/api/uv-ec8033b4f6 (auto-deleted)",
            "Inherited via uv.lock transitive bump (voyageai → langchain-text-splitters → langchain-core 1.3.3).",
            "n/a",
            "n/a",
            "n/a",
            "All checks green on dependabot PR; squash-merged ahead of Hari batch to clear pip-audit (high+critical) gate.",
            "n/a (CVE-2026-44843 closed; pip-audit green on every Hari rebased PR).",
        ],
    ]
    for r in rows:
        ws.append(r)
    _wrap_rows(ws)
    widths = [22, 8, 12, 42, 55, 50, 45, 45, 55, 55]
    for i, w in enumerate(widths, start=1):
        ws.column_dimensions[get_column_letter(i)].width = w
...
def build_pending_prod_proof_sheet(wb: Workbook) -> None:
    ...
    _add_header(ws, headers)
    rows = [
        ["BUG-032", "#24", "Merge to main", "Reviewer", "Done", "MERGED 2026-05-10 as ed01fdd."],
        ["BUG-032", "#24", "Deploy via scripts/deploy-prod.sh", "Operator", "Done 2026-05-10", "Deploy script exit 0 on ec55dc2; caseops-api revision caseops-api-00132-c6n + caseops-web revision caseops-web-00121-278 both serve image :ec55dc2; ClamAV sidecar + EG-003 guard passed; health 200."],
        ["BUG-032", "#24", "Run hari-2026-05-09-bug-032-prod.spec.ts on deployed SHA", "Prod verification (Playwright) workflow", "Done 2026-05-10", "2/2 PASS on prod ec55dc2 via workflow run 25628453223. API: POST /api/matters/{id}/court-orders mounted, disposable order created on QA tenant. UI: /app/matters renders + Add-order affordance reachable."],
        ["BUG-033", "#21", "Merge to main", "Reviewer", "Done", "MERGED 2026-05-10 as 877c615."],
        ["BUG-033", "#21", "Deploy via scripts/deploy-prod.sh", "Operator", "Awaiting deploy approval", "Deploy script exit 0."],
        ["BUG-033", "#21", "Run hari-2026-05-09-bug-033-prod.spec.ts on deployed SHA", "Prod-Playwright workflow", "Done 2026-05-10", "3/3 PASS on prod ec55dc2 via workflow run 25628453223. /account/setup + /account/reset-password render the missing-token state (NOT 404); invalid-token submission surfaces actionable error. Full happy-path with debug_token still requires DEBUG_TOKEN_ENVS broadening — separate task."],
        ["BUG-034", "#20", "Merge to main", "Reviewer", "Done", "MERGED 2026-05-10 as 78108e4."],
        ["BUG-034", "#20", "Deploy via scripts/deploy-prod.sh", "Operator", "Awaiting deploy approval", "Deploy script exit 0."],
        ["BUG-034", "#20", "Run hari-2026-05-09-prod.spec.ts on deployed SHA", "Prod-Playwright workflow", "Done 2026-05-10", "2/2 PASS on prod ec55dc2 via workflow run 25628453223. API: catalog flags non-delegable caps + custom-role create rejects with 403. UI: protected capabilities disabled in admin/roles."],
        ["BUG-038 (SendGrid)", "#22", "Merge to main", "Reviewer", "Done", "MERGED 2026-05-10 as 2b571cc."],
        ["BUG-038 (SendGrid)", "#22", "Deploy via scripts/deploy-prod.sh", "Operator", "Done 2026-05-10", "Deploy script exit 0 on ec55dc2; Alembic migration ran via caseops-migrate-job; caseops-api-00132-c6n env now references caseops-sendgrid-webhook-public-key:latest via secretKeyRef. Health 200."],
        ["BUG-038 (SendGrid)", "#22", "Create Secret Manager value caseops-sendgrid-webhook-public-key", "Operator", "Done (pre-existing)", "projects/490751771290/secrets/caseops-sendgrid-webhook-public-key version 1 ENABLED; SendGrid-provided P-256 public key already loaded by operator before this sequence began. Secret value never read by CaseOps assistant. IAM grant: roles/secretmanager.secretAccessor scoped to this single secret for SA caseops-runtime@perfect-period-305406.iam.gserviceaccount.com."],
        ["BUG-038 (SendGrid)", "#22", "Wire CASEOPS_SENDGRID_WEBHOOK_PUBLIC_KEY into Cloud Run", "Operator (CaseOps assistant)", "Done 2026-05-10", "gcloud run services update caseops-api --update-secrets=CASEOPS_SENDGRID_WEBHOOK_PUBLIC_KEY=caseops-sendgrid-webhook-public-key:latest. Verified post-deploy: env present with secretKeyRef.name=caseops-sendgrid-webhook-public-key key=latest. Fail-closed verified: invalid + missing signatures both → 401 'SendGrid webhook signature verification failed.' at canonical URL."],
        ["BUG-038 (SendGrid)", "#22", "Configure SendGrid dashboard Signed Event Webhook", "Operator (Codex)", "Done 2026-05-10", "Dashboard ON, URL exact match https://api.caseops.ai/api/webhooks/sendgrid/events, Signed Event Webhook ON; events enabled = delivered/bounce/dropped/spam_report/unsubscribe/group_unsubscribe; public_key_present=true. Verified via SendGrid API."],
        ["BUG-038 (SendGrid)", "#22", "SendGrid official webhook test API ping", "Operator (Codex)", "Done 2026-05-10", "Test API returned HTTP 204 No Content. Settings id: c3d614d6-40fa-4db7-b295-c3e6cf44cd92."],
        ["BUG-038 (SendGrid)", "#22", "Live SendGrid Event API POSTs reaching Cloud Run", "SendGrid → CaseOps", "Done 2026-05-10", "Cloud Run logs show real User-Agent='SendGrid Event API' POSTs to /api/webhooks/sendgrid/events returning HTTP 200. Samples: 2026-05-10T15:42:14Z 0.162s, 15:40:03Z 0.094s, 15:35:17Z 1.738s. Combined with the prior fail-closed 401 on invalid sig, this proves signature verification is active end-to-end."],
        ["BUG-038 (SendGrid)", "#22", "Database-side EmailSuppression row from real production event", "Operator", "PENDING — sole gating item", "A SQL probe on the prod DB showing a tenant-scoped EmailSuppression row written from one of the 200-returning POSTs above. The 200 status is strong indirect evidence (the route handler writes the row before returning), but per the bug-fixing skill we keep the conservative verdict until the row is directly observed."],
        ...
        ["BUG-039 (Outlook)", "#23", "Run hari-2026-05-09-outlook-sync-prod.spec.ts on deployed SHA", "Prod-Playwright workflow", "Done 2026-05-10", "2/2 PASS on prod ec55dc2 via workflow run 25628453223. UI: /app/calendar renders Outlook panel + hides sync button without a connection. API: POST /api/calendar/sync/outlook mounted, returns 409 without an Outlook connection."],
        ["Cross-PR", "#20/#21/#23/#24", "Resolve testMatch overlap on each rebase", "Done", "Resolved during merge sequence", "All four hari-2026-05-09-*.spec.ts entries present in playwright.app.config.ts AND playwright.prod-ram.config.ts on main as of ed01fdd."],
        ["Pre-cursor", "#18", "Merge dependabot langchain-core 1.3.3 to clear CVE-2026-44843", "Done", "Done", "MERGED 2026-05-10 as b827efb. Pip-audit gate green on every rebased Hari PR."],
    ]
    for r in rows:
        ws.append(r)
    _wrap_rows(ws)
