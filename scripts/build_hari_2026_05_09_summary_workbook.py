...
def build_summary_sheet(wb: Workbook) -> None:
    ...
    rows = [
        ...
        ["Verdict status", "BUG-032/033/034/039 → Properly fixed (prod-Playwright passed on deployed SHA). BUG-038 → Partially fixed: runtime ready, provider-side SendGrid dashboard + live signed-event verification still pending operator."],
        ...
        ["SendGrid runtime readiness", "CASEOPS_SENDGRID_WEBHOOK_PUBLIC_KEY env wired via secretKeyRef name=caseops-sendgrid-webhook-public-key key=latest on caseops-api. Fail-closed verified: invalid + missing signatures both → HTTP 401 'SendGrid webhook signature verification failed.' at canonical URL https://api.caseops.ai/api/webhooks/sendgrid/events. Secret value never read or printed."],
        ["SendGrid remaining caveats (operator)", "Provider dashboard config + live signed-event end-to-end verification per docs/runbooks/sendgrid-event-webhook.md. CaseOps assistant has no SendGrid dashboard access."],
        [],
...
def build_bug_details_sheet(wb: Workbook) -> None:
    ...
    rows = [
        ...
        [
            ...
            "Partially fixed — runtime ready on deployed SHA ec55dc2: CASEOPS_SENDGRID_WEBHOOK_PUBLIC_KEY env wired (secretKeyRef name=caseops-sendgrid-webhook-public-key key=latest); fail-closed verified — invalid + missing signatures both → 401 'SendGrid webhook signature verification failed.' at canonical URL https://api.caseops.ai/api/webhooks/sendgrid/events. Pending operator-side: SendGrid dashboard signed-webhook enablement + live signed-event end-to-end verification per docs/runbooks/sendgrid-event-webhook.md. CaseOps assistant has no SendGrid dashboard access.",
...
def build_pending_prod_proof_sheet(wb: Workbook) -> None:
    ...
    rows = [
        ...
        ["BUG-038 (SendGrid)", "#22", "Configure SendGrid dashboard Signed Event Webhook", "Operator", "PENDING — operator only", "Settings → Mail Settings → Event Webhook → enable Signed Event Webhook → POST URL https://api.caseops.ai/api/webhooks/sendgrid/events; enable bounce/dropped/spam_report/unsubscribe/group_unsubscribe events. CaseOps assistant has no SendGrid dashboard access."],
        ["BUG-038 (SendGrid)", "#22", "Run end-to-end signed webhook verification per runbook", "Operator", "PENDING — gated on dashboard config", "Per docs/runbooks/sendgrid-event-webhook.md: 200 on signed valid event; 401 on missing/invalid signature (already verified pre-dashboard); EmailSuppression row appears in tenant-scoped table after a real bounce/unsubscribe."],
