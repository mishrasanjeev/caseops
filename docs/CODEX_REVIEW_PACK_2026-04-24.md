...
Hand-off snapshot for Codex review. All 5 P0s and 9 of 10 P1s from
`docs/STRICT_REPO_QUALITY_AUDIT_2026-04-24.md` are closed; the one
remaining P1 (P1-006 Postgres-backed DB validation) needs CI service
container infrastructure that's outside today's scope.
...
| P0-003 | Calendar ICS declares `text/calendar` in OpenAPI, runtime header agrees | `tests/test_calendar.py::test_ics_openapi_media_type_matches_runtime_header` |
| P0-004 | SendGrid webhook fail-closed in non-local env (no key OR no crypto → 503) | `tests/test_sendgrid_webhook_security.py` (9 cases) |
...
| P1-002 | Route coverage matrix — `tests/test_route_coverage_matrix.py` fails when any new `/api/*` route lands without a test reference. 16 baseline waivers tracked with TODO+date. |
...
| P1-007 | AI route governance — every `/api/ai/*` and `/api/recommendations/*` mutation now has `@limiter.limit(ai_route_rate_limit)` (was missing on 7 routes in `ai.py`). Enforced by `tests/test_ai_route_governance.py` |
...
3. **P1-002 route matrix waivers (16 routes)** — explicit TODO+date entries in `tests/test_route_coverage_matrix.py::ALLOWED_UNTESTED`. Each is real test gap, not a fix-now blocker.
4. **P1-003 page matrix waivers (20 pages)** — same shape in `apps/web/app/__page-coverage-matrix.test.ts::ALLOWED_UNTESTED`.
5. **Phase C-2/C-3** (client portal screens, outside-counsel portal) intentionally paused — the Phase C-1 scaffold is in prod with full test coverage, including 19 backend portal tests + 9 web portal vitest cases + a prod-smoke clickthrough. Resuming is a deliberate decision, not a hidden gap.
...
| `tests/test_tenant_isolation.py` + 6 sibling auth tests (52 tests) | green locally — was broken on 9ea5cc2 by the cookie-auth-vs-TestClient interaction |
