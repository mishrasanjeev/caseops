...
- `IP-SCOPE-10`: M8-M10 cannot exit on schema presence. A version-controlled domain contract, complete journey inventory, machine-validated legal/source packs, migration, security, performance, support, and exact-release evidence gate the affected domain's beta/GA claim and milestone exit. Missing legal or provider authority keeps only the affected capability fail-closed; it does not block deployment or completion of the repository implementation.
...
**Actor:** Codex owns implementation and release evidence; a Specialist IP Lawyer supplies authority only for legally effectful rules or conclusions.
**Preconditions:** Proposed domain, jurisdictions, versioned source/legal pack, synthetic acceptance fixtures and target beta/GA claim are named.
**Main flow:** Codex inventories real user workflows, right-specific parties/relationships/states/forms/fees/rules/sources/providers/documents/deadlines and migration data; writes a version-controlled domain contract with normal/exception/contested/transfer/maintenance/closure journeys; threat and confidentiality controls are executable; legal fixtures and the source pack pass schema, provenance, coverage and contradiction checks; required legal authority is recorded before any fixture can drive legal state; architecture proves reuse boundaries and prevents trademark-state leakage; implementation, migration, automated acceptance and exact-release evidence pass; capability service derives the label from that evidence.
...
**Audit/postcondition:** Domain-contract version, legally effectful authorizations, source/rule packs, machine acceptance evidence, exact release, label changes and rollback remain linked.
...
|---|---|---|---|
| M0: Program contract | 14 Aug 2026 | Version-controlled scope, source policy, initial taxonomy/rule inputs, synthetic pilot fixtures, and a machine-validated `NEW/EXTEND/LINK/REPLACE` ownership ledger for every M1-M3 component | Named accountability boundaries; duplicate-subsystem and ADR validators pass; required fixture data is versioned; unresolved provider choices remain capability-scoped and fail-closed |
...
| M7: Trademark General Availability | 29 Sep 2028 | Access-review/emergency-access controls, automated export/purge/offboarding, migration toolkit, hardening, performance, accessibility, current DR/security/incident proof, support/runbooks, billing/packaging and GA rollout | UJ-28/UJ-58/UJ-63/UJ-64/UJ-65/UJ-67/UJ-68; access expiry/review, regional/failover and purge/tombstone drills; 30-day SLO evidence; zero P0/P1; exact-release production acceptance suite; support/finance readiness checks |
| M8: Patent Docket Beta | 30 Jun 2029 | Version-controlled patent-domain contract covering families, PCT/national phase, office actions/oppositions, claim/document versions, annuities/working, title/licensing and patent deadlines | UJ-29, UJ-39, UJ-40, UJ-60, UJ-61 and machine-validated patent legal fixtures pass; no trademark-rule/status leakage; beta label derives from capability evidence; legally effectful rules remain disabled until their required legal authority is recorded |
| M9: Designs, Copyright, Licensing Beta | 31 Jan 2030 | Version-controlled type-specific domain contracts covering assets, registrations, renewals, enforcement, ownership and licensing obligations | UJ-30, UJ-41 through UJ-43, UJ-60, UJ-61 and machine-validated type-specific fixtures pass; contract/registry deadline and confidentiality separation are proven; legally effectful rules remain disabled until their required legal authority is recorded |
| M10: Full IP Platform GA | 31 Jul 2032 | Machine-validated domain contracts for trademark, patent, design, copyright, GI, plant variety, layout design, trade secret, domain/customs enforcement, and licensing operations | UJ-44/UJ-45/UJ-60/UJ-61 plus every domain's specialist journeys/fixtures; cross-module security/performance/DR audit; migration, documentation, capability labels, pricing and support readiness checks |
...
- To accelerate M2-M7, parallelize dependency-independent implementation and tests while keeping architecture, migrations, access/data governance, legal rules, provider contracts and exact-release evidence serialized at their canonical owner.
- M8-M10 repository implementation may proceed in parallel behind unavailable/intake-only flags. Each domain's authoritative legal behavior remains fail-closed until its versioned legal/source pack and required legal authority exist; staffing is a planning input, not an implementation or release approval gate.
...
- Scope added after M0 must have an explicit dependency owner, bounded acceptance evidence, and a traceable slice. Capacity forecasts may move without creating an implementation approval gate.
...
- M2 activation requires the law firm's actual document-name list, an IP lawyer's initial rule/workflow/calendar inventory, data-retention/hold policy, security decisions, current backup/object/export evidence, and the machine-validated Section 11.2 ownership/retirement ledger. An unresolved shared-owner decision blocks changes to that owner, not other M2 nodes.
- M3 authoritative clearance/filing/fee behavior requires validated data and representative portfolio files. Portfolio, workflow, and UI code may be completed with versioned synthetic fixtures and remain disabled/manual until required legal or financial authority exists.
...
- M6 authoritative research/AI activation requires verified source coverage and a versioned, machine-validated evaluation/security dataset. Permission, citation, abstention, and provider-failure implementation proceeds with deterministic fixtures.
...
- Implement externally blocked behavior completely behind truthful unavailable/manual states, fail-closed flags, readiness checks, observability, and kill switches. Missing authority blocks only the exact legally, financially, externally, or destructively effectful action; it does not block repository completion or a fail-closed release.
- Repository protections, automated checks, exact-image deployment identity, and dated production E2E are the routine gates owned by Codex. Human action is limited to authority that cannot be delegated: a real legal/financial/external communication act, unavailable credentials/paid capacity, or authorization of an irreversible production data operation.
...
### 26.4 Acceptance evidence and effectful authority

| Area | Routine machine-enforced evidence | Human authority retained only for the effectful act |
|---|---|---|
| Legal rule/form/deadline fixture | Version/hash binding, complete legal fixtures, stale-write/four-eyes tests, source coverage and fail-closed activation | A named qualified lawyer authorizes a rule or legal conclusion before it can drive authoritative legal state |
| Tenant/permission/security | Cross-tenant, restricted-record, revocation, cache/session and capability parity suites | An authorized access owner decides an emergency or exceptional access grant; ordinary releases need no manual signoff |
| Step-up/four-eyes/break-glass/access review | Identity separation, expiry, scope, revocation and no-standing-grant residue tests | The product's independent emergency-access decision and retrospective review remain required |
| Retention/legal hold/export/purge/offboarding | Immutable preview manifest, hold/exclusion refresh, idempotency, restore proof and exact target validation | The tenant/records authority authorizes an irreversible customer-data operation against the unchanged manifest |
| Provider terms/source attribution | Versioned terms/source inventory, readiness probe, default-off capability and expiry enforcement | The legal/provider owner records the external permission needed to enable the affected live adapter |
| Migration reconciliation | Row/count/hash parity, exception manifest, mixed-revision proof, rollback/roll-forward and production history access | Non-anonymized destructive or irreversible tenant migration uses the data-operation authorization above |
| UX/accessibility | Automated axe, keyboard, responsive-boundary and Playwright journeys | None for implementation or release |
| Production operations/SLO/rollback | Exact commit/image/schema identity, hosted CI, health/index checks, dated production E2E and rollback rehearsal | None for an already authorized reversible deployment |
| Backup/restore/failover and worker fencing | Automated restore, RPO/RTO, tombstone, no-dual-send and unrelated-endpoint responsiveness drills | None for drills using approved test or anonymized data |
| Journey acceptance | Versioned synthetic/allowlisted fixtures and the complete production Playwright/API suite | A real filing, draft finalization, payment, client communication, or other legal effect still uses its product authorization workflow |

Machine-verifiable implementation and release evidence is sufficient to close repository work. A human decision never substitutes for a failed automated gate. Conversely, automation never fabricates authority for a real filing, legal conclusion, payment, external client communication, emergency access grant, provider licence, or irreversible customer-data action.

Effectful authorization is scoped to the exact immutable object or operation manifest and is checked immediately before execution. It does not create a milestone-wide checkpoint, a general release signoff, or a blocker for unrelated fail-closed implementation.
