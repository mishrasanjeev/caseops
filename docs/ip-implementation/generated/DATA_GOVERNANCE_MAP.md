...
- Canonical map SHA-256: `b3ef35f2e02013314a5ad18bf8645d7cf1ba079b46bb9b2b42cc2fce2fae6e3e`
- SQL tables: `304`
- SQL columns: `4930`
- ORM indexes: `1640`
- Alembic/raw index declarations: `688`
- Non-SQL data classes: `12`

## Boundary

This inventory does not claim approved retention bounds, legal-hold activation, tenant export, purge, offboarding, provider deletion, backup recovery/restore, residency, or data-governance/recovery milestone completion. The only current disposition behavior is fail-closed Definition-of-Ready validation. This release's tenant-scoped company-user lookup, deadline-coverage guards, standalone-deadline offboarding serialization, and bounded Today IP coverage stream expose existing registered SQL classes only. The Today stream is a read projection over ip_deadline_coverages, ip_docket_records, and matter_deadlines that reuses the existing IP docket access predicate; the web endpoint and generated OpenAPI contract changes add no SQL or non-SQL data class, new storage, or disposition authority. IPLF-057B backfills the required ip_trademark_particular_versions compatibility projection for existing trademark_international_registrations dockets; both tables are already registered, Madrid facts retain their canonical owner, and the bounded restore-forward migration adds no data class, storage boundary, retention authority, or disposition handler. EG-009 narrows what failure paths persist: audit-export, authority-ingestion, court-sync, document-job, document-processing, embedding and legal-update errors now store redacted text instead of raw exception strings. This removes infrastructure detail from already-registered columns; it adds no SQL or non-SQL data class, no new storage, and no disposition authority. The 2026-08-27 recurring-job hardening changes only version-controlled Cloud Run commands, arguments, environment and secret references, runtime identity, Cloud SQL attachment, resources, retries, and timeouts. It invokes already-baked application entrypoints, preserves the existing registered SQL/provider/storage boundaries and paused governance writers, and adds no data class, persistence path, provider transmission, retention authority, or disposition handler. The 2026-08-28 API capacity hardening changes only Cloud Run service-level minimum capacity, exact deployment verification, and bounded reads over existing registered Matter and IP data. It adds no SQL or non-SQL data class, persistence path, provider transmission, retention authority, disposition behavior, or data owner. IPLF-062A adds tenant-scoped assistant sessions, explicit polymorphic scope references, turns, and exact citation provenance under the tenant-restricted legal-content profile. The tenant AI policy supplies a bounded session expiry, but automated destruction remains unauthorized until the hold-aware disposition owner is implemented and approved; downgrade therefore refuses retained assistant evidence. IPLF-063 extends the existing recommendation and Draft records with tenant-bound Matter/IP targets, frozen legal-review context and source provenance, lawyer selection/finalization evidence, redacted terminal state, and an idempotent Draft link under the same tenant-restricted legal-content profile. It adds no alternate research, source, drafting, audit, model-run, billing, retention, hold, export, purge, or provider-deletion owner; downgrade refuses retained intelligent-review or linked Draft evidence. The 2026-08-28 production stabilization stores one bounded, hashed official-provider source text on the existing tracked-case update owner so a billing-unavailable certified PDF can fall back only to complete integrity-verified provider markdown; truncation or hash mismatch fails closed, snapshots retain only a bounded preview, and no alternate source, provider, retention, or disposition owner is introduced. The 2026-08-29 follow-up changes only billing-unavailable response classification for that existing bounded cache, bundles already-authorized Matter and IP document projections into the existing foreign-associate workspace read, and raises service-level warm capacity. It adds no SQL or non-SQL data class, persistence path, provider transmission, retention authority, disposition behavior, or data owner. The 2026-08-29 exact-release correction classifies provider HTTP 402 under the existing rate-limit health state and writes one zero-cost verified-cached terminal operation per exact release, linked to an existing hash-verified provider snapshot and complete tracked-case source update. It makes no provider call when valid stored evidence exists, leaves provider freshness and health unchanged, and adds no data class, persistence boundary, provider transmission, retention authority, disposition behavior, or data owner. The 2026-08-29 legacy-evidence correction repairs at most one missing tracked-case source body from an already hash-verified provider snapshot: it requires an exact update-type, source-key, and source-URL match, complete non-truncated text, empty target text and hash fields, bounded candidate work, and immutable provenance audit. It never overwrites existing source evidence, changes provider health or freshness, calls the provider, or introduces a new data class, storage boundary, retention authority, disposition behavior, or data owner. IPLF-066A adds tenant-private index generations, private content projections and typed scopes, idempotent revocation/tombstone events, and content-free saved-output access manifests under fail-closed tenant policies. The only non-SQL class is an identifier-only process cache partitioned by tenant, membership and security generations; every cache hit is reauthorized from current SQL state. It does not merge private content into the public corpus, authorize provider transmission, activate retention or purge, or claim the IPLF-066B user workflow and release proof.
...
| `drafts` | `tenant_restricted_legal_content` | 16 | `registry_fail_closed` |
...
| `ip_cost_item_corrections` | `tenant_restricted_legal_content` | 10 | `registry_fail_closed` |
...
| `ip_journal_ingestion_runs` | `tenant_restricted_legal_content` | 21 | `registry_fail_closed` |
...
| `ip_title_interests` | `tenant_restricted_legal_content` | 19 | `registry_fail_closed` |
...
| `ip_watch_handoffs` | `tenant_restricted_legal_content` | 14 | `registry_fail_closed` |
| `ip_watch_hits` | `tenant_restricted_legal_content` | 25 | `registry_fail_closed` |
| `ip_watch_profiles` | `tenant_restricted_legal_content` | 25 | `registry_fail_closed` |
...
| `private_index_generations` | `security_identity_control` | 14 | `registry_fail_closed` |
| `private_index_projection_scopes` | `security_identity_control` | 10 | `registry_fail_closed` |
| `private_index_projections` | `tenant_restricted_legal_content` | 26 | `registry_fail_closed` |
| `private_projection_events` | `security_identity_control` | 18 | `registry_fail_closed` |
| `private_saved_output_access` | `tenant_restricted_legal_content` | 16 | `registry_fail_closed` |
...
| `tracked_case_updates` | `tenant_operational_record` | 19 | `registry_fail_closed` |
...
| `tenant-private-retrieval-candidate-cache` | `cache` | `registry_fail_closed` |
