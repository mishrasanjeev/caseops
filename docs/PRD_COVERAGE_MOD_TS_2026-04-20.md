...
| Indian Kanoon live use and spend | `J05`, `M04`, `IPLF-054/056B` | Licensed human calls count toward the shared account-wide Indian Kanoon/eCourts budget and billing report; automated requests remain nonbillable. | No scraper, second corpus, cache, source action, or provider dashboard. | A configured credential is not proof of a meaningful live result. |
| Court hierarchy and aliases | `J02`, `J06`, `M02`, `M05`, Legal Workspace S4, existing matter import | Manual and bulk creation resolve the same active canonical catalog and reviewed aliases; governed platform configuration manages source/review/audit state; ambiguous import rows return canonical candidates with lineage. | Extends the existing forum catalog and importer instead of parser branches or an alternative court master. | All-India reviewed alias coverage is partial and remains a source-backed data operation. |
...
Acceptance IDs are `US-058..062`, `FT-083..093`, `NFT-022`, and
`SEC-028..030` in `docs/PRD_CODEX_2026-04-23.md`. The dated executable browser
...
and must not perform a credit-bearing request. Governed alias administration is
owned by `tests/e2e/forum-alias-admin-2026-09-04.spec.ts`.
