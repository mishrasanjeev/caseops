...
| **12** | Live-LLM drafting quality **harness** — `caseops_api.scripts.eval_drafting_quality`. Three live runs landed iteratively: **(1) 2.76/5** (harness flaws — `retrieved=[]` + template-agnostic structure rubric). **(2) 4.06/5** after the harness fix (per-template structure rubric + per-template seeded authorities + procedural-only citation rule for vakalat/caveat). **(3) 4.13/5** after prompt tightening (≥3-citations floor in the generic system prompt + explicit `FROM:` / `TO:` opening on cheque/property notices + citation hook on affidavit). Cumulative spend $2.14, well under the $5 authorisation. quashing_petition holds 5.0/5; 12 templates ≥4.33/5; weakest is criminal_complaint at 3.33/5. Remaining 0.67 gap to 4.8/5 needs deeper work — closing the validator-warnings gap (8 scenarios still at val=3.0) + exemplar few-shot shots on the lagging templates. | `d2d20c4` + iterative follow-ups | Real numbers logged; harness ready to gate future prompt work |
