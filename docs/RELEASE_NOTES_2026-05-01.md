...
| **12** | Live-LLM drafting quality **harness** — `caseops_api.scripts.eval_drafting_quality`. Five live runs landed iteratively: **(1) 2.76/5** (harness flaws). **(2) 4.06/5** after harness fix (per-template structure rubric + per-template seeded authorities). **(3) 4.13/5** after prompt tightening. **(4) 4.33/5** after validator template-awareness + few-shot exemplars. **(5) 4.36/5** after BNSS-regex fix + 3-grounds-3-cites exemplar + Arnesh Kumar seeded for anticipatory_bail. Cumulative spend **$3.65** of the $5 authorised. **3 templates at 5.0/5** (quashing_petition, vakalatnama, **anticipatory_bail ↑** — went 3.67→5.0 in run 5 alone); 13 at 4.33/5; 4 stuck at 4.00/5 (amendment, cheque_bounce, criminal_complaint, divorce_petition). Remaining 0.44 gap to 4.8/5 is structural: citation_score caps at 3.0 on most pleadings because GPT-5.1 emits 1-2 inline brackets even when explicitly told ≥3. Closing it cleanly needs a two-pass generation (forced-completion to inject the missing anchors) or a citation-supervisor wrapper. | `d2d20c4` + iterative follow-ups | Real numbers logged; harness ready to gate future prompt work |
