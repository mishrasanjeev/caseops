...
| **12** | Live-LLM drafting quality **harness** — `caseops_api.scripts.eval_drafting_quality`. Six live runs landed iteratively: **(1) 2.76/5** (harness flaws). **(2) 4.06/5** after harness fix. **(3) 4.13/5** after prompt tightening. **(4) 4.33/5** after validator template-awareness + few-shot exemplars. **(5) 4.36/5** after BNSS-regex fix + 3-grounds exemplar + 3rd seeded authority (anticipatory_bail 3.67 → 5.0). **(6) 4.41/5** after per-template structure rubrics for criminal_complaint / amendment_of_pleadings / divorce_petition + broader cheque-bounce deadline markers (all 4 laggards 4.0 → 4.33). Cumulative spend **$4.55** of the $5 authorised. **3 templates at 5.0/5** (quashing_petition, vakalatnama, anticipatory_bail); **16 at 4.33/5**; 1 at 4.0/5 (written_statement — regressed from temperature noise). Remaining 0.39 gap to 4.8/5 is structural: citation_score caps at 3.0 on most pleadings because GPT-5.1 emits 1-2 inline brackets even when explicitly told ≥3. Closing it cleanly needs a two-pass generation or a citation-supervisor wrapper. | `d2d20c4` + iterative follow-ups | Real numbers logged; harness ready to gate future prompt work |
