...
| **12** | Live-LLM drafting quality **harness** — `caseops_api.scripts.eval_drafting_quality`. Four live runs landed iteratively: **(1) 2.76/5** (harness flaws). **(2) 4.06/5** after harness fix (per-template structure rubric + per-template seeded authorities). **(3) 4.13/5** after prompt tightening (≥3-citations floor + `FROM:` / `TO:` notice opening + citation hook on affidavit). **(4) 4.33/5** after validator template-awareness + few-shot exemplars (skip `no_inline_anchors` for procedural-only templates; few-shot snippets on criminal_complaint / anticipatory_bail / reply_counter_affidavit). Cumulative spend **$2.85** of the $5 authorised. **3 templates at 5.0/5** (quashing_petition, caveat_petition, vakalatnama); 14 ≥4.33; weakest is anticipatory_bail at 3.67. Remaining 0.47 gap to 4.8/5 is now entirely citation-count — the model hedges at 1-2 cites despite explicit ≥3-cite instruction. Closing it likely needs a two-pass generation (first pass body, second pass forces ≥3 inline anchors) or a citation-supervisor on top of generation. | `d2d20c4` + iterative follow-ups | Real numbers logged; harness ready to gate future prompt work |
