...
| **12** | Live-LLM drafting quality **harness** — `caseops_api.scripts.eval_drafting_quality` — assembles the EXACT production prompt and runs each fixture through GPT-5.1 to score validator + structure + citations against the 4.8/5 PG-005 target. **First live run (2026-05-01): 2.76/5 across 24 scenarios, $0.70 spend.** See `docs/EVAL_DRAFTING_QUALITY.md` — score is depressed by two harness issues (citation score = 0 because the eval passes `retrieved=[]`; structure rubric is template-agnostic). Validator = 3.0 (warnings, no errors) on every scenario. | `d2d20c4` + first run `f8a55ae`+1 | Live numbers logged; harness fixes + prompt tightening queued |
