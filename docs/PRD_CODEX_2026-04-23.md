...
- BUG-010's September 07 source release is mapped and evidenced in
  `docs/ip-implementation/evidence/bug010-source-release-2026-09-07.md`.
  It reuses the statute/source-version/reference owners and completes the five
  reported editions' numbered inventories without fabricating legal currency.
  The user's scoped release includes accumulated changes after local acceptance;
  remaining roadmap slices keep their incomplete statuses and availability gates.
  Its September 08 follow-up additionally rejects terminal party/priority
  creation replay without removing authorized historical reads. Complete rebuilt
  local proof includes 4,173 API passes, 192 PostgreSQL passes, 274 browser passes
  and 23 existing-QA patent/domain journeys. Exact-production proof is separate;
  catalog-wide statutory coverage and all 25 unfinished IP slices remain open.
  The first exact-release production attempt exposed a real HTTPS slash-redirect
  defect and three stale acceptance contracts. The repair, independent failure
  analysis and fresh local gates are recorded in
  `docs/ip-implementation/evidence/bug010-production-followup-2026-09-08.md`.
  That follow-up does not certify production until the repaired canonical
  release passes deployed acceptance and post-QA maintenance.

- Patent priority execution detail is in
  `docs/ip-implementation/patent-priority-execution-2026-09-07.md` (UJ-29,
  PAT-01, M02/M08/M13/M14). Current source extends `IpRelationship` with
  versioned evidence and application/family views. Focused verification is in
  `docs/ip-implementation/evidence/local-patent-priorities-2026-09-07.md`;
  the newer b525 runtime includes the bounded ancestry correction and passes
  all three priority widths. The 7ec22 checkpoint's 178 PostgreSQL and 968
  frontend passes remain historical; the evidence record separates current
  focused proof from complete-suite claims. Complete release acceptance
  remains blocked by BUG-010, incomplete scope and unfulfilled gates. This does
  not activate authoritative patent automation or complete IPLF-080.

- `docs/PRD_IP_LAW_FIRM_PLATFORM_2026-08-01.md` and its canonical
  `docs/ip-implementation/PROGRAM_MANIFEST.yaml` govern the IP program.
  Codex owns all pending slices as of 2026-09-06. The locally verified domain
  catalogue, unpublished-disclosure boundaries and restricted family/application
  intake with immutable source-pinned corrections and sourced parties are partial IPLF-079/080
  implementation, not completed patent support. Patent child
  stories and unimplemented journeys are detailed in
  `docs/PRD_PATENT_DOMAIN_2026-09-06.md`; current evidence and all 25 remaining
  slices are reconciled in `docs/ip-implementation/codex-takeover-2026-09-06.md`.
...
| `MOD-TS-006` | Calender | `J08`, `M08`, `US-022/023/024/025/057`, `FT-042/043/078-082` | `Partial` | The verified automatic next-hearing slice is implemented: approved case-tracking providers refresh exact court identities at 18:00 IST, update the existing matter field without prediction, and expose the same rules through Sync Now. Full calendar UI and notification-channel depth remain incomplete. |
...
September 05 additive identifier contract (`RAM05-ECASE`, M02, US-004/005):
retain an optional Temporary E-Case Number independently from Filing Number,
Case Number and CNR. Create, edit, bulk import and matter search must round-trip
it. Adding final identifiers must preserve it on the same matter. Existing
tenant ACLs, audit, optimistic concurrency and terminal-state guards apply;
temporary identifiers must never be submitted as a Case Number/CNR to a provider.
Acceptance includes search before and after final registration, old-client
PATCH preservation, no automatic lifecycle change, migration safety and
desktop/mobile browser proof. See `docs/bugfix-ram05sep-2026.md`.
...
6. At 18:00 IST, a bounded case-tracking run discovers eligible old and new
   matters, resolves each matter through one active approved court/provider
   scope, and writes the nearest verified non-past next-hearing date into the
   existing `next_hearing_on` field.
7. A permitted user can invoke Sync Now for one tracked case through the same
   matching, lifecycle, and audit rules used by the scheduled run.
...
- automatic case tracking uses CNR as the primary exact identity. Without CNR,
  require one canonical court and a provider-published registration, filing or
  case-number identity, corroborated against all supplied type/year, location
  and party fields. Party names or case titles alone are insufficient. Missing
  court identity, conflicting evidence, multiple or truncated candidates,
  not-found, provider mismatch and invalid identifiers must not mutate the
  matter. Do not discard a conflicting CNR and silently fall back to a weaker
  search. This extended BUG-014 contract is pending implementation and proof.
- provider failure and confirmed no-upcoming-hearing are different states:
  transient failure retains the last valid date, while a confirmed absence may
  clear only an unlocked provider-managed date
- past provider dates are ignored and no predicted date may be substituted
- terminal matters are excluded; a hearing refresh must never dispose, reopen,
  activate, or increment the matter lifecycle version
- scheduled, bulk, Docker, and regular Playwright runs must use the deterministic
  provider emulator or the no-paid-provider marker; a live paid probe is a
  separate, explicit, bounded release action

Done when:

- hearing preparation and deadlines are controlled from one reliable flow
- old and new eligible matters converge idempotently on one verified upcoming
  hearing date without duplicate bookmarks or concurrent refresh operations
...
| M15 | Data platform and enterprise deployment | Partial | Voyage production path, OpenAI-assisted enrichment, rerank, air-gap |
...
- Corpus metadata enrichment for titles, citations, parties, judges, benches,
  sections, and format labels MUST use the high-reliability OpenAI extraction model or an approved
  equivalent high-reliability path before embedding.
- Where the production workflow uses OpenAI-backed enrichment or evaluation,
...
6. OpenAI-assisted title, citation, party, judge, bench, and section cleanup
...
- `US-025` As a partner, I want hearing packs that reflect the latest case state
  and authorities.
- `US-057` As a legal operator, I want old and new matters with a reliable court
  identity to receive the nearest verified upcoming hearing date automatically
  at 18:00 IST, without prediction, duplicate fields, or lifecycle changes.
...
- `FT-078` The 18:00 IST scheduled poll discovers a bounded set of eligible old
  and new matters, creates at most one bookmark per matter, and updates the
  existing `next_hearing_on` field from the nearest verified non-past date.
- `FT-079` Exact CNR and exact case-number-plus-court matching accept one unique
  result only; not-found, ambiguity, mismatch, invented identifiers, and past
  dates perform zero matter-date writes.
- `FT-080` Provider failure retains the last valid date; a confirmed absence
  clears only an unlocked provider-managed date and never erases a manual lock.
- `FT-081` Concurrent scheduled and Sync Now refreshes are idempotent, create no
  duplicate running operations or bookmarks, and report a typed concurrency
  result instead of racing a write. A case-number identity that learns an
  already-canonical CNR converges automatically without a unique-key failure or
  manual replay.
- `FT-082` Disposed matters are excluded, and successful automatic or manual
  refresh leaves status, `is_active`, and lifecycle version unchanged after
  reload while the Matters and Case Tracking pages show the same date.
...
- `NFT-021` Automatic next-hearing work is bounded by candidate and batch caps,
  bulk provider calls, provider deadlines, bounded retries, cooldowns, and
  query-count regressions; one slow or abandoned provider request must not
  starve an unrelated endpoint.
...
- `SEC-027` Automatic next-hearing sync uses only an active approved provider
  catalog scope and server-side secrets, rejects ambiguous or invented court
  identity before a credit-bearing call, performs no scraping or CAPTCHA
  bypass, and honors the no-paid-provider marker in every runtime.

### 14.4 Ram 2026-09-04 provider, forum, and Bare Act acceptance addendum

This addendum extends existing owners only. Indian Kanoon research remains in
`J05/M04`; eCourts search, tracking, and hearing sync remain in `J03/J08/M02/M08`;
tenant and platform spend remain in `J11/J14/M10/M14`; court identity remains in
`J02/J06/M02/M05`; and Bare Act trust remains in `J05/M05/MOD-TS-017` and
`IPLF-006`. It creates no parallel provider dashboard, billing ledger, court
master, matter importer, statute catalog, source proxy, or lifecycle writer.

Additional user stories:

- `US-058` As an authenticated human user, I can use a configured licensed
  provider even when my workspace slug looks like a QA slug; explicit automated
  requests remain nonbillable.
- `US-059` As a workspace administrator, I can see current-month Indian Kanoon
  and eCourts spend, limit, remaining amount, currency, and unlimited status in
  the existing billing usage report.
- `US-060` As an intake operator, I can enter a reviewed court name or alias in
  manual or bulk matter creation and receive one canonical forum result, while
  ambiguous court-complex names require jurisdiction context.
- `US-061` As a researcher, I can open every catalogued Bare Act section from
  an Act detail page and see its honest verification state and source, without
  unverified text becoming selectable legal evidence.
- `US-062` As an authorized platform catalog curator, I can create, review,
  update, and deactivate source-backed court aliases without a code release;
  intake operators see bounded canonical candidates instead of a guessed court
  when an upload remains ambiguous.

Additional functional tests:

- `FT-083` The authoritative `X-CaseOps-Automated-Test: no-paid-providers`
  marker blocks Indian Kanoon and eCourts searches, detail retrievals,
  refreshes, polling, and source downloads before transport or spend; readiness
  and CaseOps-recorded balance endpoints remain readable.
- `FT-084` A human request in a test-looking tenant is not blocked by its slug.
  Scheduled polling still excludes configured test tenants, and local pytest
  cannot reach a paid host unless a deterministic transport is injected.
- `FT-085` Each account receives one shared INR 1,000 monthly limit across
  Indian Kanoon and eCourts by default. A tenant-first atomic reservation
  includes settled current-month spend and unexpired reservations from every
  provider in that shared scope, and returns 429 before transport when the
  account limit would be exceeded.
- `FT-086` Explicit active database policies can make one provider unlimited
  for one account. The founder-authorized GBA Law Office and Pinelabs Pvt. Ltd.
  policies are seeded for both providers without a runtime company-name bypass.
- `FT-087` Every successful credit-bearing search, refresh, scheduled refresh,
  and source retrieval records exactly one provider-keyed usage event and
  attribution. Failed, blocked, cached, and deterministic-test paths do not
  inflate spend; partial scheduled batches settle only successful call cost.
- `FT-088` Indian Kanoon health and eCourts status expose shared-account or
  provider budget scope, monthly spend, limit, remaining amount, currency,
  policy source, and unlimited state without an external balance probe.
- `FT-089` Forum resolution order is exact canonical name, reviewed active
  alias, then normalized contextual match. State, district, level, and category
  narrow candidates; zero or multiple candidates are never guessed. Manual and
  bounded 500-row bulk workflows use the same catalog and preserve original
  input.
- `FT-090` An Act detail page lists verified, pending, quarantined, and retired
  catalog sections with source navigation and truthful state. Only sections
  carrying the complete verified source contract are selectable for matters,
  drafting, or AI grounding.
- `FT-091` Matter read serialization is side-effect free. Generic reads,
  imports, children, provider sync, and metadata updates cannot reopen a
  terminal matter; only the dedicated optimistic-concurrency lifecycle route
  may perform an audited controlled reopen.
- `FT-092` The forum-alias registry is capability- and recent-step-up-protected,
  stores alias type, canonical entry, source evidence, review state, active
  state, actor attribution, timestamps, and optimistic record version, and
  rejects unverified resolution, missing verified evidence, stale writes,
  normalized duplicates, and canonical collisions with typed errors.
- `FT-093` Manual and bulk resolution use the same active catalog and verified
  alias rows. An ambiguous bulk row remains invalid and returns a bounded,
  structured list of canonical candidate IDs, names, and lineages in both the
  preview and error export; no candidate is silently selected.

Additional non-functional and security tests:

- `NFT-022` Provider-spend reads use company/provider/time indexes; reservation
  checks lock one tenant, expire in bounded time, and do not hold that lock over
  external I/O. PostgreSQL migration and index checks must pass before release.
- `SEC-028` Unlimited access is granted only by a tenant/provider policy row,
  never by request-supplied tenant names, slugs, headers, or UI state.
- `SEC-029` Provider usage and policy projections are tenant-scoped. Tenant
  reports expose their own operational spend but never platform-wide internal
  cost or another account's activity.
- `SEC-030` Tenant roles cannot read or mutate the global alias registry.
  Platform alias mutations require the dedicated catalog capability, recent
  security step-up, an operator reason, and an immutable platform audit event.
...
- No vector-data feature may claim production readiness unless Voyage,
  OpenAI-assisted enrichment, reranker-on retrieval, and the 4.8+/5 quality gate
  are satisfied.
