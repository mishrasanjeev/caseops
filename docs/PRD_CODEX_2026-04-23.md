...
- `FT-085` Each account receives a separate INR 1,000 monthly limit for Indian
  Kanoon and eCourts by default. A tenant-first atomic reservation includes
  settled current-month spend plus unexpired reservations and returns 429
  before transport when the limit would be exceeded.
...
- `FT-088` Indian Kanoon health and eCourts status expose monthly spend, limit,
  remaining amount, currency, policy source, and unlimited state without an
  external balance probe.
