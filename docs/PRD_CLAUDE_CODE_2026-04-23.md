...
| `MOD-TS-001-A` | Appeal Strength Analyzer | `J06`, `J07`, `J09`, `M05`, `M06`, `M07`, `US-014/015/017/018/021/027`, `FT-024B/031B` | `Shipped` | Per-ground argument-completeness analysis on appeal_memorandum drafts. Pure-read service (`services/appeal_strength.py`), GET `/api/matters/{id}/appeal-strength` route, AppealStrengthPanel UI rendered alongside BenchContextCard. Structural no-favorability sweep at every layer (in-service `_check_phrase` + 8 backend tests + 2 vitest). Shipped 2026-04-25 (commit `62a3b66`). |
| `MOD-TS-001-B` | Judge Career History | `J06`, `M05`, `US-014C/015C`, `FT-024C-1..5` | `Shipped` | `judge_appointments` table + per-judge career timeline on `/app/courts/judges/{id}` with source-attributed evidence. SC backfilled from `parent_high_court` (31 current + 4 prior HC); Delhi HC backfilled with bio prose from per-judge profile pages (32 rows). Cloud Run Jobs `caseops-backfill-sc-judge-career` + `caseops-backfill-delhi-hc-judge-career` ran cleanly 2026-04-25 (commits `6f8857f`, `88d6883`). |
| `MOD-TS-001-C` | Bench → Judge FK Resolver | `J06`, `J08`, `M05`, `US-014D/015D`, `FT-024D-1..6` | `Shipped` | `MatterCauseListEntry.judges_json` + `services/bench_resolver.py` parses free-text bench rosters, resolves court-scoped to Judge FKs at the high-quality confidence floor (initial+surname AND court-scope, OR full name AND court-scope). Cloud Run Job `caseops-resolve-cause-list-benches` runs nightly. Matter hearings UI renders `resolved_bench` as clickable per-judge links. Shipped 2026-04-25 (commits `b8a6bf9`, `a424460`). |
| `MOD-TS-001-D` | Bench-Specific BAAD Context + Advocate-Bias | `J06`, `J07`, `M05`, `M06`, `US-014E/015E`, `FT-024E-1..6` | `Shipped` | When an appeal_memorandum draft is generated for a matter with an upcoming listing, BAAD pulls authorities authored by THAT bench specifically (not just the court) and applies advocate-bias selection: prefers authorities matching the matter's practice area. New `bench_specific_authorities` block on the prompt + UI section in BenchContextCard. Per PRD §16.3 strategic differentiation. Shipped 2026-04-25 (commits `69ef7b1`, `ede88a2`). |
| `MOD-TS-001-E` | Tolerant Judge Alias Matcher | `J06`, `M05`, `US-014F`, `FT-024F-1..4` | `Shipped` | `judge_aliases` table + `services/judge_aliases.py` resolves "Justice A.K. Sikri" to "Adarsh Kumar Sikri" without ILIKE fragility. 269 aliases live across 63 judges (avg 4.3 per judge). Read-only admin page at `/app/admin/judge-aliases`. Cloud Run Job `caseops-backfill-judge-aliases`. Shipped 2026-04-25 (commits `60566e3`, `06f7df0`). |
...
| `MOD-TS-013` | Clients Verification | `J12`, `M11`, `US-037`, `FT-049` | `Shipped` | KYC lifecycle live: `clients.kyc_status/kyc_verified_at/kyc_verified_by_membership_id` columns (Phase B M11 slice 3, alembic `20260423_0004`), `verify_client_kyc` service + POST route, `/app/clients/{id}` page with KYC submit + review actions gated by `clients:kyc_submit` / `clients:kyc_review` capabilities. Audit row written on every KYC state transition. |
| `MOD-TS-014` | Portal Persona Model + Shared Scaffold | `J17`, `J18`, `M18`, `US-050/051`, `FT-070/071` | `Shipped` | Phase C-1 (2026-04-24, alembic `20260424_0001`): `portal_users` table, magic-link auth on `/portal/*` cookie scope (`caseops_portal_session` + `caseops_portal_csrf`), `MatterPortalGrant` scoping, branded portal shell. SendGrid invitation send wired in production. |
| `MOD-TS-015` | Client Portal | `J17`, `M19`, `US-052/053`, `FT-072/073` | `Shipped` | Phase C-2 (2026-04-24, commit `b0965e9`): six `/api/portal/*` endpoints + web pages `/portal` (matter list) + `/portal/matters/[id]` (Overview / Comms / Hearings / KYC tabs). 14 backend tests cover tenant isolation, no-grant 404, can_reply gate, audit on reply + KYC submit. |
| `MOD-TS-016` | Outside Counsel Portal | `J18`, `M20`, `US-054/055`, `FT-074/075` | `Shipped` | Phase C-3 (2026-04-24, alembic `20260424_0002`): assigned-matter view + work-product upload + invoice submission + time entries, gated by `MatterPortalGrant.role='outside_counsel'`. Verified live with `tests/e2e/oc-portal.spec.ts` (passing in 15.3s, 2026-04-25). |
| `MOD-TS-017` | Statute + Bare Acts Model | `J05`, `J07`, `M05`, `US-046A/B/C/D`, `FT-S1-1..5/S2-1..7/S3-1..6/S4-1..4` | `Shipped` | Closes WTD-7.4. Schema (`statutes` + `statute_sections` + `matter_statute_references` + `authority_statute_references`, alembic `20260425_0004`); seed loader for 7 central acts (BNSS, BNS, BSA, CrPC, IPC, Constitution, NI Act — 91 sections live, source-attributed to indiacode.nic.in); read API + `/app/statutes` browser; tolerant section parser (`services/statute_resolver.py`, BNSS-vs-BNS substring trap handled, '§' / 'S.' / 'Article' variants); matter cockpit Statutes sub-tab + drafting prompt extension that injects bare section text for verbatim quoting on appeal-memorandum drafts (cap 600 chars per section). 23 backend tests + 5 vitest. Shipped 2026-04-25 (S1 `a91e561`, S2 `65b7b6e`, S3 `9fb364b`, S4 `5c6050a`, CI fixes `c274007`). |
