...
| BUG-019 / BUG-025 (calendar empty) | **Properly fixed** (2026-04-30, commit `f641e4b`) | Workflow-as-fix shipped: matter cockpit `/app/matters/[id]` now renders an empty-state Card with a "+ Add hearing" CTA when `upcomingHearings.length === 0`. CTA opens the same `ScheduleHearingDialog` (now extracted to `apps/web/components/matters/ScheduleHearingDialog.tsx` and shared with the Hearings sub-tab). On submit, the matter workspace query is invalidated, the empty-state hides, and the new hearing surfaces in `/api/calendar/events`. Populated matters keep the BUG-011 invariant (no ghost empty cards). Evidence: `tests/e2e/recommendations-grounding-2026-04-29-prod.spec.ts:148` PASSED on `caseops-web-00083-jqj` (`f641e4b`) — fresh matter → click "+ Add hearing" → fill dialog → assert empty-state hides + `/api/calendar/events` returns the new hearing. |
...
| BUG-015 (recommendations hang / 504) | **Properly fixed** (2026-04-30, commit `da7216a`) | Root cause: `services/authorities._pg_prefilter_document_ids` did `GROUP BY document_id ORDER BY MIN(<=>)` — the planner can't push HNSW under GROUP BY, so under concurrent INSERT load (citation extraction + EN sweep) it fell back to scanning all 1.8M chunks and stalled past Cloud Run's 300s budget. Fix: rewrite as nested CTEs — inner CTE uses `ORDER BY <=> LIMIT chunk_limit` (HNSW-served O(log n)), outer CTE filters + aggregates on the small candidate set. Evidence: `tests/e2e/recommendations-grounding-2026-04-29-prod.spec.ts:148` ("BUG-015 root cause stress matter") PASSED on deployed `caseops-api-00087-5b6` (`da7216a`) — the previously-hanging stress matter `31f0577f-ea2e-4033-b16b-d04e16b13729` returned within 110s. The 60s `SET LOCAL statement_timeout` band-aid stays as defense-in-depth. |
