...
| BUG-024 / BUG-033 / BUG-034 (recommendations 422) | **Partially fixed — new failure mode** | The original "model returned citations, but none matched verified authorities" rejection no longer fires (verified via real-prod probe). BUT the workflow still fails with HTTP 502: both Anthropic Haiku and OpenAI fallback return JSON missing the required schema keys (`['title', 'options', 'primary_recommendation_label', 'rationale', 'assumptions', 'missing_facts', 'confidence', 'next_action']`). Per brutal-memory Pattern 2, replacing one failure with another is `Partially fixed`. Track as **BUG-035: recommendations 502 — LLM JSON schema mismatch** for next session. |
