...
test.describe.serial("IPLF-054 licensed Indian Kanoon research", () => {
  ...
  test("default-off API and browser state make no provider call", async ({
    page,
  }) => {
    ...
    const readinessMessage = page.getByTestId(
      "research-indian-kanoon-readiness",
    );
    await expect(readinessMessage).toContainText(
      "No provider call will be made",
    );
    await expect(readinessMessage).toContainText(
      /setup is incomplete|disabled by the runtime switch/,
    );
    await expect(readinessMessage).not.toContainText("INDIAN_KANOON_");
    await page
