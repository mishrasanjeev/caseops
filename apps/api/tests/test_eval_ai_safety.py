...
def test_docs_status_marks_wtd114_as_foundation_partial() -> None:
    ...
    assert "`WTD-11.4` `Partially implemented`" in strict
    assert "WTD-11.4 AI safety tests -- PARTIAL foundation" in wtd
    assert "foundation only" in runbook.lower()
    assert "does not call live LLM providers" in runbook
