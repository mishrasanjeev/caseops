...
def test_legacy_enrichment_never_calls_indian_kanoon_public_html(
    client: TestClient,
) -> None:
    """Indian Kanoon is reachable only through the licensed API adapter."""
    factory = get_session_factory()
    with factory() as session:
        st, sec = _seed_statute_and_section(session, sec_no="999")
        sec_id = sec.id
    ...
    assert result.source is None
    requested_urls = [call.args[0] for call in client_inst.get.call_args_list]
    assert requested_urls == ["https://example.test/handle/1/2"]
