...
indexes, and append-only triggers; no existing application rows are scanned
or rewritten.
...
def downgrade() -> None:
    ...
    retained = op.get_bind().execute(
        sa.text("SELECT count(*) FROM ip_filing_transactions")
    ).scalar_one()
    if retained:
    ...
    for index_name in (
        ...
        op.drop_index(index_name, table_name="ip_filing_transactions")
