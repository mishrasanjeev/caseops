...
def activate_legal_hold(
    ...
    *,
    ...
    """Delegate legacy callers to the authenticated independent hold writer."""

    # Compatibility callers cannot nominate an absent approver. The HTTP
    # command and this adapter delegate to the same authenticated writer.
    from caseops_api.services.legal_hold_workflow import activate_hold

    if approver_membership_id != context.membership.id:
        raise HTTPException(
            status_code=403, detail="The signed-in person must perform the approval."
        )
    del approver_label
    retained = session.get(LegalHold, hold_id)
    if retained is None or retained.company_id != context.company.id:
        raise HTTPException(status_code=404, detail="Legal hold not found.")
    activate_hold(
        session, context=context, hold_id=hold_id, expected_updated_at=retained.updated_at
    )
    return retained
...
def release_legal_hold(
    ...
    *,
    ...
    """Retired nomination-based command; release requires an immutable proposal."""

    raise HTTPException(
        status_code=503,
        detail="Release requires an immutable proposal and authenticated independent approval.",
    )
