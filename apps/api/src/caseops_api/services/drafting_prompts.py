...
_BAIL = PromptParts(
    system=(
        ...
        "Court practice.\n"
        "INLINE-CITATION RULE: when ≥3 authorities are supplied in the "
        "AUTHORITIES block, the Grounds section MUST contain at least "
        "three distinct bracketed inline anchors (e.g. "
        "`[(2020) 5 SCC 1]`). Every citation listed in the JSON "
        "`citations` field MUST also appear inline at least once.\n"
...
_ANTICIPATORY_BAIL = PromptParts(
    system=(
        ...
        "INLINE-CITATION RULE — STRICT: when ≥2 authorities are "
        "supplied, the Grounds section MUST contain at least three "
        "distinct bracketed inline anchors. The third anchor MAY "
        "repeat a supplied authority on a separate doctrinal point if "
        "fewer than three distinct authorities are available. "
        "Citations in the JSON `citations` field MUST also appear "
        "inline. Fewer than 3 inline anchors fails the production "
        "validator and is rejected.\n"
        "MANDATORY: the Grounds section MUST contain THREE numbered "
        "grounds (Ground A, Ground B, Ground C), each anchored to a "
        "DIFFERENT bracketed authority from the AUTHORITIES block. "
        "Two grounds is insufficient — three is the floor. If you "
        "have only two supplied authorities, repeat one for a "
        "separate doctrinal point in Ground C.\n"
        "FEW-SHOT EXEMPLAR for the Grounds section (count the "
        "anchors — there are EXACTLY 3):\n"
        "  'Ground A: The applicant has reasonable apprehension of "
        "  arrest within the meaning of BNSS s.482. The test under "
        "  Sibbia [(1980) 2 SCC 565] is satisfied since the FIR "
        "  discloses no flight risk.\n"
        "  Ground B: Anticipatory bail granted in this matter would "
        "  not be time-limited per Sushila Aggarwal [(2020) 5 SCC 1]; "
        "  the protection subsists till the end of trial.\n"
        "  Ground C: The applicant has cooperated and is entitled to "
        "  s.41A notice protections under Arnesh Kumar [(2014) 8 SCC "
        "  273]; custodial interrogation is unnecessary.'\n"
        "PRE-RETURN SELF-CHECK (do this before emitting JSON):\n"
        "  1. Does the body contain the literal string 'BNSS s.482' "
        "     OR 'Section 482' with 'BNSS' nearby? If no, add it.\n"
        "  2. Count the bracketed [...] anchors in the body. If "
        "     fewer than 3, add a third anchor by introducing one "
        "     more doctrinal sentence supported by a supplied "
        "     authority.\n"
        "  3. Every entry in the JSON `citations` array MUST appear "
        "     at least once as a bracketed anchor in the body.\n"
        "REQUIRED STRUCTURE:\n"
...
_CRIMINAL_COMPLAINT = PromptParts(
    system=(
        ...
        "INLINE-CITATION RULE: every entry in the AUTHORITIES block "
        "MUST appear at least once as a bracketed inline anchor in "
        "the body (e.g. `[(2012) 5 SCC 424]`). The validator flags "
        "`citation.coverage_gap` if any emitted citation never appears "
        "inline; do not list a citation in the JSON `citations` field "
        "without anchoring it in the body too.\n"
        "FEW-SHOT EXEMPLAR for the prayer paragraph:\n"
        "  'In view of the foregoing, the Complainant prays that this "
        "  Hon'ble Court be pleased to take cognisance of the offences "
        "  under BNSS s.223 read with BNS s.318 [BNS s.318] and BNS "
        "  s.337 [BNS s.337], summon the Accused under BNSS s.227 in "
        "  light of the Magistrate's duty to apply mind at the "
        "  cognisance stage [(2012) 5 SCC 424], and proceed in "
        "  accordance with law.'\n"
        "REQUIRED STRUCTURE:\n"
...
_CIVIL_SUIT = PromptParts(
    system=(
        ...
        "INLINE-CITATION RULE: when ≥3 authorities are supplied, the "
        "Cause of Action / Grounds section MUST contain at least three "
        "distinct bracketed inline anchors. Citations in the JSON "
        "`citations` field MUST also appear inline.\n"
...
_APPEAL_MEMORANDUM = PromptParts(
    system=(
        ...
        "INLINE-CITATION RULE: when ≥3 authorities are supplied, the "
        "Grounds section MUST contain at least three distinct bracketed "
        "inline anchors. Citations in the JSON `citations` field MUST "
        "also appear inline at least once.\n"
        "REQUIRED STRUCTURE (in order):\n"
...
_WRITTEN_STATEMENT = PromptParts(
    system=(
        ...
        "INLINE-CITATION RULE: when ≥3 authorities are supplied (typically "
        "limitation / Order VIII timeline cases), the preliminary "
        "objections section MUST contain at least three distinct "
        "bracketed inline anchors.\n"
        "REQUIRED STRUCTURE:\n"
...
_REPLY_COUNTER_AFFIDAVIT = PromptParts(
    system=(
        ...
        "CITATION GUIDANCE: counter-affidavits are predominantly factual, "
        "BUT when a paragraph denies a legal proposition advanced by the "
        "petition AND a relevant authority is supplied in the AUTHORITIES "
        "block, anchor that paragraph with a bracketed inline citation. "
        "Order XIX CPC + Indian Oaths Act 1969 may be cited verbatim "
        "without bracket-tags (they are statutory recitals, not "
        "argument).\n"
        "FEW-SHOT EXEMPLAR for a paragraph rebutting a legal ground:\n"
        "  '7. The contents of paragraph 12 of the writ petition are "
        "  denied. The submission that the impugned notification "
        "  violates Article 14 is misconceived; the test laid down in "
        "  [(1978) 1 SCC 248] requires arbitrariness on the face of "
        "  the record, which the petitioner has not pleaded.'\n"
        "REQUIRED STRUCTURE:\n"
...
_AMENDMENT_OF_PLEADINGS = PromptParts(
    system=(
        ...
        "INLINE-CITATION RULE: when ≥3 authorities are supplied (typically "
        "Vidyabai / Revajeetu Builders / parallel Order VI Rule 17 "
        "decisions), the Reason section MUST contain at least three "
        "distinct bracketed inline anchors. The due-diligence pleading "
        "MUST cite Vidyabai by name when supplied.\n"
        "REQUIRED STRUCTURE:\n"
