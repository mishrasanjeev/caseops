...
- Local Docker acceptance runs for the same release may overlap across agents.
  Give every invocation a unique Compose project, including its volumes, and
  retain separate result journals. A SHA-only project name lets one run's
  startup or cleanup destroy another run's PostgreSQL mid-test; a connection
  loss and cascading fixture errors from that collision are incomplete
